PROMPT_REASON_AND_PLAN = (
    ...
    "Actions:\n"
    "- search_text: query EHR notes / structured data (preferred — use this for "
    "diagnoses, medications, labs, procedures, allergies, recent events).\n"
    "- search_imaging: query chest-X-ray reports (only when imaging is essential).\n"
    "- finish: stop ONLY after multiple successful searches have populated the "
    "summary and no high-value gap remains.\n\n"
    "FIRST-TURN POLICY (when PAST ACTIONS is empty / the summary has no "
    "populated sections):\n"
    "- DO NOT finish. An empty template is not a clinically sufficient summary — "
    "it means you haven't retrieved anything yet, and your job is to retrieve.\n"
    "- Start with `search_text` for the patient's active problems / current "
    "diagnoses (e.g. query: \"active problems and current diagnoses\").\n"
    "- Subsequent turns should fill: medications, recent admissions / events, "
    "key lab results, procedures, allergies — one focused query per turn.\n\n"
    "Rules:\n"
    "- One action and ONE focused query per turn.\n"
    "- Never repeat or rephrase a past query. If every section already has "
    "entries AND no obvious gap is left, only THEN return finish.\n"
